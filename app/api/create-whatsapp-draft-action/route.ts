...
Create a WhatsApp message draft.
...
`,
    ...
    const draft =
      completion.choices[0]?.message?.content || "No WhatsApp draft created.";

    const { error: actionError } = await supabase.from("agent_actions").insert({
      agent_id: agentId,
      action_type: "whatsapp_draft",
      title: `WhatsApp Draft: ${purpose}`,
    ...
    return NextResponse.json(
      { error: "Could not create WhatsApp draft." },
      { status: 500 }
    );
  }
}